...
best-effort probe for that session's upstream interfaces that runs
`/system/bin/dumpsys ipsec`. When the probe completes, each newly observed
matching IPv4 tunnel forwarding-policy request is emitted to one currently
active session call whose upstream set still contains the target interface.
Repeated probes that observe the same target do not emit another request.
No-match is quiet;
`dumpsys` or parser failures are structured nonfatals tied to one currently
active session call for the probed interfaces, if any. The daemon clears its
emitted-target record when the target disappears from a later probe or its
interface leaves all session upstream sets. The daemon does not separately
...
Kotlin reports that either upstream role has a new upstream snapshot. Client and
downstream-only changes do not trigger a probe by themselves.
